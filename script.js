...
function openAttachmentViewer(id) {
  ...
  const stage =
    document.getElementById('attachment-stage');
...
document.addEventListener('click', (e) => {
  ...
  const img =
    document.getElementById('active-image');
  ...
  if (e.target.id === 'zoom-out') {
    ...
    currentZoom = Math.max(0.2, currentZoom);
...
attachmentViewer.addEventListener('click', (e) => {
  ...
  const inner =
    document.querySelector('.attachment-viewer-inner');
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const btn =
    document.querySelector('.attachment-close');
