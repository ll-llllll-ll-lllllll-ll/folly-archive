...
const attachmentRegistry = {
  ...
  'radio-score': {
    ...
    type: 'graphic score',
    mode: 'image',
    src: 'attachments/aether-scorched-earth/score-2.png',
    desc: '以太共振場圖形記譜。'
  },
  ...
  'plague-scan': {
    ...
    type: 'graphic score',
    mode: 'image',
    src: 'attachments/effluent-sedimentation/score-1.png',
    desc: '死水沉積圖形記譜。'
  },
...
video.ontimeupdate = () => {
  // 假设你有一个名为 currentVideo 的视频元素，并且有 timeupdate 监听器
currentVideo.addEventListener('timeupdate', () => {
  // 1. 获取视频当前播放进度 (0 到 1 之间的浮点数)
  ...
  if (arcsContainer) {
    // 3. 设定扩散的缩放范围
    const minScale = 1;   // 视频开始时的大小 (1倍)
    const maxScale = 4.5; // 视频结束时的大小 (例如放大到4.5倍，根据你的视觉需求调整)
    ...
    // (可选体验优化): 随着不断扩散，让外围边缘稍微变得透明
    // arcsContainer.style.opacity = 1 - (progress * 0.5); 
  }
});
